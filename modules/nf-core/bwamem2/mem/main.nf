...
//        | samtools $samtools_command $args2 -@ $task.cpus -o ${prefix}.bam -
    //make index a variable it needs the fa

    //consider maybe to index here i dont know.
    // CHECK IF IT JUST NEEDS THE DIRECTORY TO BE AROUND.... 
    """
    INDEX=`find -L ./ -name "*.amb" | sed 's/\\.amb\$//'`
    cp -r *.fastp.fastq.gz bovine_index/chr25/
    cd bovine_index/chr25/ &&  
    /opt/conda/bin/bwa-mem2 mem -t $task.cpus $index $reads > ${prefix}.sam
